...
public class HoursPicker extends GridPane implements Initializable {
	...
	HoursPicker(DateTimePickerPopup parentContainer) {
		...
		// Load FXML
		try {
			final FXMLLoader fxmlLoader = new FXMLLoader(new File("src/datetimepicker/HoursPicker.fxml").toURI().toURL());
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		amPmButton.selectedProperty().addListener(new ChangeListener<Boolean>() {
			@Override
		...
		hoursToggleGroup.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
			@Override
